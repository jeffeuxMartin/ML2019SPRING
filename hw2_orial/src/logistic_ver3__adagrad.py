...
parser.add_argument("-eps2", "--epsilon2", 
                       help="Adagrad epsilon term outside the sqrt.", 
                             dest="eps2", default="1e-8", type=float)
...
parser.add_argument("-b2", "--beta2", help="Adam momentum term 2.", 
                             dest="beta2", default="0.9", type=float)
...
prev_grad = np.zeros((dim + 1,))

for it in range(iterations):
    # icon = shutil.get_terminal_size().columns                       \
    #            - (10+7 + 3+7 + 8+(2+5) + 9+(1+12) + 10+(2+5)+ 2) - 29
    icon = 20
    try:
        ...
        # print("\rIteration {0:6d} / {1:6d} : lr = {2:7.6e}, "
        #       "loss = {3:8.7f}, Score = {4:.5f}  ".format(
        #          it + 1, iterations, 
        #            sum((lr / (np.sqrt(prev_grad + eps1) + eps2))**2), 
        #              loss / num, acc), end='')
        print("\rIteration {0:6d} / {1:6d} : ".format(
            it + 1, iterations), end='')
        # print("lr = {:7.6e}, ".format(
        #    sum((lr / (np.sqrt(prev_grad + eps1) + eps2))**2)), end='')
        ...
        w_b -= lr * grad / (np.sqrt(prev_grad + eps1) + eps2)
        if it % icon == (icon - 1):
            # print()
