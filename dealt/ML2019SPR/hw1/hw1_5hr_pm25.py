...
if os.path.isfile(args.train) and (args.mode == 'tr' or args.mode == 'train'):
    ...
    data = pm25.reshape(1, 12, 20, 24).transpose(1, 2, 3, 0).reshape(12, 480, 1)
    # data with 12 x 480 x 18
    
    feat_x, feat_y = [], []
    for month in data:  # month with 480 x 18
        for hour in range(480 - 6 + 1): # 0 ~ 469
            feat_x.append(month[hour:(hour + 6 - 1)])
            feat_y.append(month[hour + 6 - 1][0])
    ...
    prev_gra = np.zeros((1 * 5 + 1, 1)) # adagrad
    prev_loss = math.inf
...
with open(args.test, 'r', encoding='big5', newline='') as f:   
    tdata = np.array([[eval(tdata) if tdata != 'NR' else 0 
       for tdata in r[6:]] for r in csv.reader(f)])
tdata_c = tdata.reshape(-1, 18, 5).transpose(1, 0, 2)
# if feat_scal_u:
