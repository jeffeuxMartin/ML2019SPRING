...
import argparse
import os
def now_name(): return time.strftime(
    "%m%d_%H%M%S", time.gmtime(time.time() + 8*60*60))
now_n = now_name()
...
parser.add_argument('--testage', '-te', '--te',
                    dest='te', help='', default='')
parser.add_argument('--models', '-m', '--md',
                    dest='md', help='', default='.')
...
for file in files:
    ...
    with open(args.rs + '/' + \
        file.split('/')[-1].split('.h5')[0] + \
        'prediction_%s.csv' % now_n, 'w') as f:
            f.write('id,label\n')
