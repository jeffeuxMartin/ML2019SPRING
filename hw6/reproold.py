...
parser.add_argument('dict',type=str, help='[Input] Your jieba dict.txt.big')
# parser.add_argument('dict', type=str, help='[Input] Your test_x.csv')
parser.add_argument('output', type=str, help='[Output] Your pred.csv')
...
te = pd.read_csv(args.test_X)['comment']
jieba.load_userdict(args.dict)
def tokenize(sentence): return jieba.lcut(sentence)
P = Pool(processes=4); te = P.map(tokenize, te); P.close(); P.join()


def predictioner(mod, emb):
    print('predict model {}...'.format(mod))
    model_0 = load_model(mod)
    vecmd_0 = Word2Vec.load(emb)

    word2index_0 = {w: n for n, w in enumerate(vecmd_0.wv.vocab)}
    word2index_0['<UNK>'] = len(word2index_0)
    word2index_0['<PAD>'] = len(word2index_0)

    def turn_indx(sent, pad_sz=None):
        if pad_sz == None: 
            pad_sz = args.seq_len
        return [word2index_0.get(w) or word2index_0['<UNK>'] \
                for w in sent[:pad_sz]] \
                + [word2index_0['<PAD>']] * (pad_sz - len(sent))

    tev = np.vstack([turn_indx(s) for s in te])

    Y = model_0.predict(tev) 
    
    return Y  

# prediction_best_20190509_094139__20190509_093754.csv
# a day ago by Jeff Chen

# prediction_best_20190509_094139__20190509_093754
# 0.74880
# 0.75530
# prediction_best_20190509_095448__20190509_095147.csv
# a day ago by Jeff Chen

# prediction_best_20190509_095448__20190509_095147
# 0.74970
# 0.75480
# prediction_best_20190509_083705__20190509_083412.csv
# a day ago by Jeff Chen

# prediction_best_20190509_083705__20190509_083412
# 0.75170
# 0.75180
# prediction_best_20190509_071240

modelpairs = [
    ["model_best_20190509_094032__20190509_093745.h5",
        "saved_20190509_093745.model"],
    ["model_best_20190509_094139__20190509_093754.h5",
        "saved_20190509_093754.model"],
    ["model_best_20190509_095828__20190509_095536.h5",
        "saved_20190509_095536.model"],
    ["model_best_20190509_095448__20190509_095147.h5",
        "saved_20190509_095147.model"]
    ]

yy = [predictioner(_mod, _emb) for _mod, _emb in modelpairs]
with open(args.output, 'w') as f: 
    f.write('id,label\n') 
    for n, k in enumerate([1 if prb >= 0.5 else 0 for prb in sum(yy)/4]): 
        f.write('{},{}\n'.format(n, k))
# import time as t
...
#         self.model.add(GRU(self.args.hidden_dim, dropout=(1-self.args.dropout), 
